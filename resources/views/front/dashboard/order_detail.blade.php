                  ...
                  @foreach($order_meta as $order_meta)
                    <tr>
                      ...
                      <td>{{str_replace("_"," ",$order_meta->status)}}
                        <br>
                          <?php if($order_meta->status =='exchange'){ ?>
                              <a href="{{URL::to('/exchage-item/'.$data->id.'/'.$order_meta->id)}}">Exchange With</a>
                         <?php  }?>
                          ...
                          @if($data->shipped_date!="0000-00-00")
                              ...
                              <?PHP
                              $date1=strtotime(date("Y-m-d H:i:s", strtotime('+16 hours',strtotime($data->shipped_date))));
                              $date2=strtotime(date("Y-m-d H:i:s"));
                              if($date1>$date2)
                              {
                              if($order_meta->is_exchange == 1 && $order_meta->exchange_status!=1 && $order_meta->return_status!=1){
                              ?>
                              <a href="javascript:void(0)"  onclick="exchange_now(this.id)" id="<?=$order_meta->sub_order_id?>,<?=$data->id?>,<?=$order_meta->id?>" class="btn btn-primary exchange_btn">Exchange Now</a>
                              <?PHP
                              }
                              $check=Helper::get_order_exchange_status($data->id,$order_meta->id);
                              if($check){
                                  if($check->is_approved == 0){
                                      echo "<p style='color:coral'>Exchange is in process</p>";
                                  }
                                  else
                                  {
                                      echo "<p style='color:green'>Exchange Approved</a>";
                                  }
                              }
                              }
                        ...
                        <td><?php if($order_meta->status =='delivered'){
                            ...
                            if($getProductRating){?>
                            <i class="fa fa-star text-warning" aria-hidden="true"></i>{{$getProductRating->rating}}
                            <?php  }else{
                            ?>
        ...
        <div class="col-md-12 col-sm-12">
          <div class="panel panel-default">
            ...
            <div class="panel-body">
              ...
              <div class="row"><span class="col-md-3 col-sm-3"><strong>Total</strong> </span> <span class="col-md-9 col-sm-9"><strong><i class="fa fa-inr" aria-hidden="true"></i>{{($data->total_amount+$data->shipping_charge)}}</strong></span></div>
                   <?php
                  $walletAmount = $data->wallet_amount;
                  ?>
                    <div class="row"><span class="col-3">Wallet Amount Use</span> <span class="col-3">- <i class="fa fa-inr" aria-hidden="true"></i>{{$walletAmount}}</span></div>
                    <div class="row"><span class="col-3"><strong>Payable Amount</strong> </span> <span class="col-3"><strong><i class="fa fa-inr" aria-hidden="true"></i>{{($data->total_amount+$data->shipping_charge) - $walletAmount}}</strong></span></div>
