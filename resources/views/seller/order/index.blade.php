...
<div id="rightSidenav" class="right_side_bar">
    <div class="tab">
        <a class="focus" href="{{URL::to('seller/order')}}"><button class="tablinks"  id="defaultOpen">Pending</button></a>
                        ...
                        <div class="border relative">
                            <div class="flex-container clearfix">
								...
								<?PHP 
								 //echo $vs->order_id;
								// echo Auth::user()->id;
								//dd(Helper::get_order_image($vs->order_id,Auth::user()->id,'pending'));
                               ...
                               <?PHP endforeach; ?> 
                                ...
                                <div class="order-product-info">
                                    ...
                                    <div class="qty"><label>Qty:</label> <span><?=Helper::get_number_of_qty((!is_null($vs->order)?$vs->order->id:''),Auth::user()->id,'pending');?></span></div> 
                                      <div class="inline-block product-view-address positon-inhrit"><button class="address-view-button box-shadow" onclick="display_address(<?=(!is_null($vs->order)?$vs->order->id:'')?>)">View Address</button></div>
									  <div class="inline-block product-view positon-inhrit"><a href="{{URL::to('seller/order-details')}}/{{!is_null($vs->order)?$vs->order->id:''}}">View Details</a></div>
                            ...
                            <div class="order-product-cta-block relative clearfix">
                                <div>
                                    <button class="order-product-cta order-ship-product button-shadow button_width_50" type="button" onclick="assign_to_rider(this.id)" id="{{!is_null($vs->order)?$vs->order->id:''}},{{!is_null($vs->order)?date('d-m-Y',strtotime('+5 days', strtotime(date('d-M-Y,h:i a', strtotime($vs->order->created_at))))):''}}"> Assign To Rider </button>
	...
	var order_id= id;
	 ...
	 $(".loader_div").show();
	   $.ajax({
        ...
        url: BASE_URL+'/seller/assign-to-rider',
        type: 'POST',
