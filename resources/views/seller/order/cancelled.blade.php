...
<div id="rightSidenav" class="right_side_bar">
    <div class="tab">
        ...
        <a class="focus" href="{{URL::to('seller/cancelled-order')}}"><button class="tablinks">Cancelled</button></a>
     ...
     <div id="cencel" style="display:block;" class="tabcontent">
        ...
        <div class="order-view">
            <div>
                <div class="order-view-products">
                    <div class="order-product relative">
                        <div class="border relative">
                            <div class="flex-container clearfix">
                                <div class="order-product-image cursor-pointer">
								<?PHP 
								foreach(Helper::get_order_image($vs->order_id,Auth::user()->id,'cancelled') as $ks=>$image):
								?>
                               ...
                               <?PHP endforeach; ?> 
                                ...
                                <div class="order-product-info">
                                   ...
                                   <div class="order-div"><label>Order id :</label><span>{{$vs->order->order_id}}</span></div>
                                    ...
                                    <div class="order-div"><label>Number Of Products :</label> <span><?=Helper::get_number_of_product($vs->order->id,Auth::user()->id,'cancelled');?></span></div>
                                    <div class="order-div"><label>Total Amount :</label> <span>{{$vs->amount}} Rs.</span></div>
                                    <div class="qty"><label>Qty:</label> <span><?=Helper::get_number_of_qty($vs->order->id,Auth::user()->id,'cancelled');?></span></div> 
