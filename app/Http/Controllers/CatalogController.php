...
class CatalogController extends Controller
{
    ...
    public function index()
    {
		...
		if(Auth::user())
		{
		  ...
		  $catatlog_list=Product::with('product_image','main_category','sub_category','super_sub_category','product_note','product_item')->where('status',1)->where('user_id',Auth::user()->id)->orderBy('id','desc');	
		  ...
		  return view('seller.catalog.index',compact('catatlog_list','weekOfdays','sponsor_plan'));
		}
		else
		{
	...
	public function create()
	{
		...
		$productList=Product::where('is_admin_approved',1)->where('user_id',Auth::user()->id)->get();
		return view('seller.catalog.create',compact('category_list','brand','colorList','productList'));
	}
	public function store(Request $request)
	{
		...
		$sellerData = UserKyc::where('user_id',Auth::user()->id)->first();
		$product_info=array(
		...
		$product->save();
		//update info....
		$category= Category::select('slug')->where('id',$category)->first();
	...
	function edit($id=null,Request $request)
	{
		if(Auth::user())
		{
			...
			return view('seller.catalog.edit',['colorList'=>$colorList,'product_details'=>$product_details,'category_list'=>$category_list,'brand'=>$brand,'duplicateProduct'=>$duplicateProduct,'productList'=>$productList]);
		}
		else
		{
			return redirect('/seller/login');
	...
	public function update(Request $request)	
	{
		...
		//if seller add similar product ......
		 if($duplicae_product ==1){
			 ...
			 $weight = json_decode($request->input('weight'));
			 $price= json_decode($request->input('price'));
			 $qty= json_decode($request->input('qty'));
			 $offer= json_decode($request->input('sprice'));//it's offer in amount.
			 ...
			 echo json_encode(array('type'=>1,"status"=>true,"message"=>"Catalog has been updated successfully"));
			 exit();
		 }
		//end similar product code.......
		 ...
		 $price= json_decode($request->input('price'));
		 $qty= json_decode($request->input('qty'));
		 $offer= json_decode($request->input('sprice'));//offer in %.
		 $item_id= json_decode($request->input('item_id'));
   ...
   return imagettftext($image, $size, $angle, $x, $y, $textcolor, $fontfile, $text);
	...
	function details(Request $request)
	{
		...
		return view('seller.catalog.catalog_details_ajax',compact('catatlog_details'));
	}
	...
	function getProductList(Request $request){
		...
		$result =[];
		$data= Product::where('is_admin_approved',1)->where('user_id', '<>', Auth::user()->id)->where('name', 'like','%'. $search . '%')->get();
	...
	function duplicateCatlogDelete($id=null)
	{
		if(Auth::user())
		{
			$product=SellerDuplicateProduct::where('id', '=',$id)->first();
			...
			Session::flash('success_message', 'Catalog has been deleted successfully');
			return redirect('seller/duplicate-product');
		}
		else
		{
			return redirect('/seller/login');
	...
	function csvToArray($filename = '', $delimiter = ',')
{
	...
	{	
	    try{
			   if($request->hasFile('product_csv'))
				...
				$file = public_path("/uploads/csv/".$photo_name);
				$customerArr = $this->csvToArray($file);
				$pdata['user_id']=Auth::user()->id;
				$pdata['category_id']=$request->input('category');
				$pdata['sub_category_id']=$request->input('sub_category_id');
				$slug=Category::select('slug')->where('id',$pdata['category_id'])->first();
				$sub_category_slug= $slug->slug;
				$pdata['category_slug']=$sub_category_slug;
				 
				$slug=SubCategory::select('slug')->where('id',$pdata['sub_category_id'])->first();
				$sub_category_slug= $slug->slug;
				$pdata['sub_category_slug']=$sub_category_slug;

				$super_sub_category= $request->input('super_sub_category_id');
				if($super_sub_category)
				{
					$slug=SuperSubCategory::select('slug')->where('id',$super_sub_category)->first();
					$pdata['super_sub_category_slug']=$slug->slug;
					$pdata['super_sub_category_id']=$request->input('super_sub_category_id');
				}
					
				for ($i = 0; $i < count($customerArr); $i ++)
				{
					
					$pdata['name']=$customerArr[$i]['ProductName'];
					$pdata['slug']=$customerArr[$i]['ProductName'];
					//$pdata['brand_id']=$customerArr[$i]['Brand'];
					//$pdata['slug']= str_slug($request->name." ".$id,"-");
					$pdata['description']=$customerArr[$i]['Description'];
					$pdata['p_gst']=$customerArr[$i]['Gst'];
					$product = new Product($pdata);
		            $product->save();
		            
					$itemWeight= explode(",",$customerArr[$i]['Weight']);
					$itemMrp= explode(",",$customerArr[$i]['Mrp']);
					$itemDiscount= explode(",",$customerArr[$i]['Discount']);
					$itemQty= explode(",",$customerArr[$i]['Qty']);
					foreach($itemWeight as $ks=>$vs)
					{
						$dValue= $itemMrp[$ks]*$itemDiscount[$ks]/100;
						
						$itemArray=array();
						$itemArray['product_id']= $product->id;
						$itemArray['seller_id']= Auth::user()->id;
						$itemArray['weight']= $vs;
						$itemArray['price']= $itemMrp[$ks];
						$itemArray['offer']= $itemDiscount[$ks];
						$itemArray['qty']= $itemQty[$ks];
						$itemArray['sprice']= $itemMrp[$ks]-$dValue;
						$obj1 = new ProductItem($itemArray);
					    $obj1->save();
					}
	...
	function display_seller_suggestion(Request $request)
	{
		$search = $request->input('term');
